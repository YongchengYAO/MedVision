...
| `vllm_qwen3vl` | B | smart-resize, 32-divisible (patch 16, not 14); probe reads `image_grid_thw × patch_size` |
| `vllm_minimax_m3` | B | smart-resize, 28-divisible; probe reads `image_grid_thw × patch_size` (remote-code `MiniMaxM3VLImageProcessor`, `trust_remote_code=True`, tight `max_pixels=451584`) |
| `lingshu` | B | same Qwen2VL smart-resize; processor runs client-side |
| `vllm_glm4v` | B | smart-resize, 28-divisible; probe reads `image_grid_thw × patch_size` (GLM-4.6V/-Flash `Glm46VImageProcessor`, transformers ≥5.2.0) |
...
| `vllm_qwen3vl` | [transformers qwen3_vl](https://huggingface.co/docs/transformers/model_doc/qwen3_vl) · [Qwen3-VL-32B-Thinking card](https://huggingface.co/Qwen/Qwen3-VL-32B-Thinking) | Same `image_grid_thw × patch_size` dynamic-resolution lineage, but the checkpoint's `preprocessor_config.json` declares `Qwen2VLImageProcessorFast` with `patch_size: 16` (not 14), so the factor is **32**. |
| `vllm_glm4v` | [transformers glm4v](https://huggingface.co/docs/transformers/model_doc/glm4v) · [GLM-4.6V card](https://huggingface.co/zai-org/GLM-4.6V) | GLM-4.6V's `Glm46VImageProcessor` (transformers ≥5.2.0) shares the Qwen2VL-style smart-resize: `image_grid_thw × patch_size`, 28-multiple, no pad. (`Glm4vImageProcessor` in 4.57.x is the older GLM-4.1V/4.5V class.) |
| `vllm_minimax_m3` | [MiniMax-M3 card](https://huggingface.co/MiniMaxAI/MiniMax-M3) · resize anchor: [qwen2_5_vl](https://huggingface.co/docs/transformers/model_doc/qwen2_5_vl) | Checkpoint's remote-code `MiniMaxM3VLImageProcessor` (`image_processor.py`, no transformers built-in) is a Qwen2VL-style `smart_resize`: 28-multiple, `image_grid_thw × patch_size`, no pad — with a tight `max_pixels=451584` (~672²). The card publishes no resize math, so the `image_processor.py` source is authoritative. |
...
- **Qwen3-VL** (`vllm_qwen3vl`; alias `qwen3vl`) — `_process_img_qwen3vl`: same `image_grid_thw × patch_size` mechanism, config-driven — but the factor is **32, not 28**. `Qwen/Qwen3-VL-32B-Thinking` reuses the *same processor class* as Qwen2.5-VL (`image_processor_type: Qwen2VLImageProcessorFast`) with `patch_size(16) × merge_size(2) = 32`, so each side rounds to a multiple of 32 (no spatial padding, `perceived == content`). The probe reads `patch_size` off the processor rather than hardcoding it, so it follows whatever the checkpoint declares — but do not read "same mechanism as Qwen2.5-VL" as "28-divisible".
- **MiniMax-M3** (`vllm_minimax_m3`; alias `minimax_m3`) — `_process_img_minimax_m3`. MiniMax-M3 (`MiniMaxM3SparseForConditionalGeneration`, sparse MoE ~428B) ships its image processor as **remote code** — `MiniMaxM3VLImageProcessor` in the checkpoint's `image_processor.py`, registered via the `preprocessor_config.json` `auto_map` — so the probe must load it with `AutoImageProcessor.from_pretrained(model_hf, trust_remote_code=True)` (unlike the Qwen/GLM processors, which are built into transformers). Verified against that source, its `smart_resize` is shape-identical to Qwen2.5-VL's: each side rounded to a multiple of `patch_size(14) × merge_size(2) = 28`, **no spatial padding**, recovered as `image_grid_thw[1:3] × patch_size` — so `perceived == content`, correct per-axis for non-square inputs. The one M3-specific twist is a **tight `max_pixels = 451584` (~672²)** baked into the processor (vs Qwen's ~12.8M-px default), so M3 perceives slices at lower resolution; the processor applies that cap itself, leaving the probe identical. The `preprocessor_config`'s `process_image_mode="dynamic_res"`, `max_image_resolution=1008` and `patch_merge` keys affect only the **token count** (the processor wrapper's patch-merge compression), not the spatial resize, so they never enter the perceived-size math. See [Model-Hardware-Requirements](Model-Hardware-Requirements.md) for the vLLM-architecture-support caveat.
- **GLM-4.6V family** (`vllm_glm4v`; alias `glm4v`) — `_process_img_glm4v`. GLM-4.6V (`Glm4vMoeForConditionalGeneration`, MoE ~106B) and GLM-4.6V-Flash (`Glm4vForConditionalGeneration`, dense ~9B) share GLM-4.6V's `Glm46VImageProcessor` (declared in the checkpoint's `preprocessor_config.json`). That class is **transformers ≥5.2.0 only** — on 4.57.x `AutoImageProcessor` finds just the GLM-4.1V/4.5V `Glm4vImageProcessor` and raises *"Unrecognized image processor"* (the runtime needs transformers 5.x + vLLM 0.19.x; see [Model-Hardware-Requirements](Model-Hardware-Requirements.md)). The processor is the Qwen2VL-derived smart-resize: each side rounded to a multiple of `patch_size(14) × merge_size(2) = 28`, **no spatial padding** (`merge_size` from the config), recovered as `image_grid_thw[1:3] × patch_size`. Because the probe runs the **real** processor (`AutoImageProcessor.from_pretrained`), the perceived size is exactly what the checkpoint emits — `perceived == content` (non-padding), correct per-axis for non-square inputs, same as the Qwen rows above.
...
- **Strategy [B](#strategy-b--input-dependent-perceived-size)** — `vllm_qwen25vl`, `vllm_qwen25vl_tooluse`, `vllm_qwen3vl`, `vllm_minimax_m3`, `vllm_glm4v`, `lingshu`, `vllm_llama_3_2_vision`, `vllm_llava_onevision`, `vllm_gemma4`, `vllm_internvl3`, `llava_med`, `huatuogpt_vision`, `healthgpt`
