...
from medvision_bm.utils.configs import RANDOM_BOX_SIMULATIONS
from medvision_bm.utils.parse_utils import (
    cal_DSC, cal_IoU, cal_metrics_detection_task, cal_Precision, cal_Recall,
    convert_numpy_to_python, get_labelsMap_imgModality_from_seg_benchmark_plan,
    get_subfolders, get_targetLabel_imgModality_from_biometry_benchmark_plan,
...
def calculate_summary_metrics_per_anatomy_detection_task_for_randomModel(grouped_data, num_simulations):
    """
    Calculate summary metrics for each anatomy group.
    ...
    for parent_class, task_data in tqdm(
        grouped_data.items(),
...
def process_parsed_file_in_model_folder(
    ...
    output_path = os.path.join(
        parsed_files_dir, "summary_values_per_boxImgRatio.json"
    )
    ...
    output_path = os.path.join(
        parsed_files_dir, "summary_metrics_per_boxImgRatio.json"
    )
    with open(output_path, "w") as f:
...
def _process_task_directory(task_dir, limit, skip_model_wo_parsed_files=False):
    ...
    # Exclude "random_detection" from model directories
    model_dirs = [d for d in model_dirs if os.path.basename(
        d) != "random_detection"]
    ...
    if not os.path.exists(random_model_path):
        ...
        output_path = os.path.join(
            random_model_path, "summary_values_per_boxImgRatio.json"
        )
        with open(output_path, "w") as f:
            json.dump(convert_numpy_to_python(
                grouped_random_data), f, indent=2)
        print(
        ...
        output_path = os.path.join(
            random_model_path, "summary_metrics_per_boxImgRatio.json"
        )
        with open(output_path, "w") as f:
            json.dump(convert_numpy_to_python(
                summary_random_metrics), f, indent=2)
        print(
            f"Saved summary metrics for random detection model to {output_path}")
    # -----------
