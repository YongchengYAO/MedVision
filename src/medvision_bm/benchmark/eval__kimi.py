...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Kimi model code. For --api_provider moonshot, use a Moonshot model ID "
            "(e.g. kimi-k2.6), see https://platform.moonshot.ai/docs. "
            "For --api_provider openrouter, use an OpenRouter model ID (e.g. moonshotai/kimi-k2.6), "
    ...
    parser.add_argument(
        "--api_provider",
        default="moonshot",
        choices=["moonshot", "openrouter"],
        type=str,
        help="API provider to access the Kimi model: 'moonshot' (direct) or 'openrouter'.",
    )
    parser.add_argument(
        "--model_name",
        required=True,
...
def main():
    ...
    if not os.environ.get(api_key_env_var, "").strip():
        raise EnvironmentError(
            f"{api_key_env_var} is not set. Export it before running the Kimi evaluation with --api_provider {args.api_provider}."
        )

    # Configuration
    kimi_model_code = args.kimi_model_code
    ...
    if not args.skip_env_setup:
        ...
        install_vendored_lmms_eval(proj_dependency="kimi")
    ...
    for task in tasks:
        ...
        # model configuration for Kimi
        # NOTE: model_hf carries the raw model code; the evaluator injects it into
        # lmms_eval_specific_kwargs, and kimi.kimi_resized_hw() normalizes it
        # ("moonshotai/" prefix stripped) for cap lookup -- normalization lives
        # in exactly one place (kimi._normalize_model_code).
        model_args = (
            f"model={kimi_model_code},"
            f"provider={args.api_provider},"
            f"model_hf={kimi_model_code},"
