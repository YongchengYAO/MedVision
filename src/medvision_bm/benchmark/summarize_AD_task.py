...
def cal_metrics_AD_task(results):
    ...
    if not success:
        # Same gate as summarize_TL_task, for the same reason: without it, the
        # precomputed branch below returns the record's stored nMAE on rows this
        # parser rejected. That value was written at eval time by the pre-1b812f5
        # whole-response parser and passed through untouched by parse_outputs, so
        # it survives on rows the <answer>-scoped parser behind ``success``
        # rejects (measured on AD-v2-CoT, 2026-08-14: 5,047 of 20,181 rejected
        # rows). The ratio is lower than TL's because AD only sets nmae_success
        # for metric_type == "distance": among rejected rows, non-distance
        # targets carried a live nMAE in exactly 0 cases.
        nmae = np.nan
        nmae_success = False
    ...
    elif doc_meta is not None and doc_meta.get("metric_type") == "distance":
...
def _update_metric_counters_AD_task(metrics_dict, counters):
    ...
    nmae_entry = metrics_dict.get("nMAE", {})
    if nmae_entry.get("success") and np.isfinite(nmae_entry.get("NMAE", np.nan)):
        counters["sum_NMAE"] += nmae_entry["NMAE"]
...
def calculate_summary_metrics_per_anatomy_AD_task(all_data, processes=None):
    ...
    for item in all_data:
        ...
        if label not in grouped_data:
            grouped_data[label] = {"targets": [], "responses": [], "doc_metas": []}

        grouped_data[label]["targets"].append(target)
        grouped_data[label]["responses"].append(response)
        grouped_data[label]["doc_metas"].append(doc_meta)
    summary_metrics = {}
...
def process_combined_jsonl_files(jsonl_paths, limit, resps_key="filtered_resps"):
    """
    ...
    for jsonl_path in jsonl_paths:
        file_data = process_jsonl_file(jsonl_path, limit, resps_key=resps_key)
...
def process_parsed_file_in_model_folder(
    ...
    assert_resps_key(
        parsed_files_dir,
        [p for paths in grouped_files.values() for p in paths],
        resps_key,
    )
    ...
    for group_name, file_paths in grouped_files.items():
        if len(file_paths) == 1:
            ...
            file_data = process_jsonl_file(file_paths[0], limit, resps_key=resps_key)
        else:
            # Combined file processing (e.g., multiple FeTA24 files)
            file_data = process_combined_jsonl_files(
                file_paths, limit, resps_key=resps_key
            )
    ...
    if not all_data:
        print(
            ...
            f"(resps_key={resps_key!r}), skipping..."
        )
        return
...
def print_model_summaries(
    ...
    output_filename = (
        f"summary_AD_task{'_limit' + str(limit) if limit is not None else ''}"
    ...
    for model_dir in model_dirs:
        ...
        # Process metrics: each label maps to {avgMAE, avgMRE, avgNMAE, SuccessRate, MRE<k, num_samples}
        nmae_valid_samples = 0
        for label, label_metrics in metrics.items():
            mae = label_metrics.get("avgMAE")
            mre = label_metrics.get("avgMRE")
            nmae = label_metrics.get("avgNMAE")
        ...
        if nmae_valid_samples > 0:
            model_summary["weighted_avg_nmae"] = (
                model_summary["weighted_sum_nmae"] / nmae_valid_samples
            )
    ...
    for model, summary in model_summaries.items():
        ...
        weighted_avg = (
            ...
            f"nMAE: "
            f"{(summary['weighted_avg_nmae'] if summary['weighted_avg_nmae'] is not None else float('nan')):.4f}, "
            f"SR: "
        ...
        def calculate_group_avg(group_labels):
            ...
            for l in group_labels:
                ...
                if l.get("nMAE") is not None and not np.isnan(l["nMAE"]):
                    weighted_sum_nmae += l["nMAE"] * samples
                    valid_samples_nmae += samples
            ...
            return {
                ...
                "nMAE": (
                    weighted_sum_nmae / valid_samples_nmae
                    if valid_samples_nmae > 0
                    else float("nan")
                ),
                "SR": (
        ...
        group_header = "\nGroup averages:"
        group_table_header = (
            f"{'Group':<15} | {'MAE':<8} | {'MRE':<8} | {'nMAE':<8} | {'SR':<8} | "
        ...
        table_header = (
            f"{'Label':<50} | {'MAE':<8} | {'MRE':<8} | {'nMAE':<8} | {'SR':<8} | "
            f"{'MRE<0.1':<8} | {'MRE<0.2':<8} | {'MRE<0.3':<8} | {'Samples':<8}"
        )
        separator = "-" * 139
        ...
        for label, m in sorted_labels:
            ...
            line = (
                ...
                f"{(nmae if nmae is not None else float('nan')):<8.4f} | "
