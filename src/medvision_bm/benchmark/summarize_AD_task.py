...
from medvision_bm.utils.parse_utils import (
    ...
    get_labelsMap_imgModality_from_biometry_benchmark_plan, get_subfolders,
...
def _calculate_final_metrics_AD_task(counters, count_total):
    ...
    task_metrics = {
        ...
        "avgMRE": counters['sum_MRE'] / counters['count_valid_RE'] if counters['count_valid_RE'] > 0 else np.nan,
    ...
    for key in keys:
        for k in range(1, 11):
            ...
            task_metrics[f"M{key}<{k/10:.1f}"] = cumulative_count / \
...
def calculate_summary_metrics_per_anatomy_AD_task(all_data):
    ...
    for task_type, data in grouped_data.items():
        ...
        task_metrics = _calculate_final_metrics_AD_task(
            counters, count_total)
        summary_metrics[task_type] = task_metrics
...
def process_jsonl_file_TL_task(
    ...
    """
    Process a JSONL file and extract relevant fields for Tumor/Lesion (TL) task evaluation.
    
    **Note: This function is currently not used in the AD task workflow. It's kept for 
    potential future TL task integration.**
    ...
    Returns:
        list: List of tuples containing:
            (imgModality, planner_task_type, label_name, target, 
             filtered_resps, task_id, slice_dim)

    Raises:
        ValueError: If JSON parsing fails or task type is unknown
    """
    results = []
    ...
    with open(jsonl_path, "r") as f:
        for _, line in enumerate(f):
            ...
            try:
                ...
                # Determine planner task type from filename
                ...
                if (
                    ...
                    # Get label mapping and image modality
...
def process_combined_jsonl_files(
    jsonl_paths, limit
):
    """
...
def print_model_summaries(task_dir, skip_model_wo_parsed_files=False):
    ...
    for model_dir in model_dirs:
        ...
        if wcount_re01 > 0:
            model_summary["weighted_mre_lt_01"] = wsum_re01 / wcount_re01
        if wcount_re02 > 0:
            model_summary["weighted_mre_lt_02"] = wsum_re02 / wcount_re02
        if wcount_re03 > 0:
            model_summary["weighted_mre_lt_03"] = wsum_re03 / wcount_re03
    ...
    for model, summary in model_summaries.items():
        ...
        acc_parts = []
        if summary["weighted_mre_lt_01"] is not None:
            acc_parts.append(
                f"Weighted MRE<0.1: {summary['weighted_mre_lt_01']:.4f}")
        if summary["weighted_mre_lt_02"] is not None:
            acc_parts.append(
                f"Weighted MRE<0.2: {summary['weighted_mre_lt_02']:.4f}")
        if summary["weighted_mre_lt_03"] is not None:
            acc_parts.append(
                f"Weighted MRE<0.3: {summary['weighted_mre_lt_03']:.4f}")
        ...
        table_header = (
            f"{'Label':<50} | {'Task Type':<10} | {'MAE':<8} | {'MRE':<8} | {'SR':<8} | "
            f"{'MRE<0.1':<8} | {'MRE<0.2':<8} | {'MRE<0.3':<8} | {'Samples':<8}"
        )
        separator = "-" * 128
