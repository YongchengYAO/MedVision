...
def install_transformers_for_gemma4(transformers_version="5.5.0"):
    # NOTE: Gemma 4 requires transformers >= 5.5.0 (config.transformers_version = "5.5.0.dev0"),
    # which is newer than any version vLLM 0.19.0 pins (transformers<5, see vLLM issue #39216).
    # Install it AFTER vLLM so it overrides vLLM's pin. A stable >=5.5.0 release is on PyPI
    # (latest 5.10.x); once the exact working version is validated, pin it here and in the
    # requirements file.
    subprocess.run(
        [
            sys.executable,
...
def parse_args():
    ...
    # toggle Gemma 4 reasoning ("thinking") mode
    parser.add_argument(
        "--enable_thinking",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Enable Gemma 4 reasoning mode (passed through to the model's chat template). Use --no-enable_thinking to disable.",
    )
    parser.add_argument(
        "--reshape_image_hw",
        default=None,
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "Maximum context length (prompt + generation) for the vLLM engine. "
            "Gemma 4's native 262144 (256K) context needs ~33 GiB of KV cache, which does not "
            "fit on a single 80 GB GPU alongside the ~59 GiB of weights. Capping it to the "
            "MedVision workload (image ~2.5K tokens + prompt + max_new_tokens) lets it run on one GPU."
        ),
    )
    parser.add_argument(
        "--stop_strings",
...
def main():
    ...
    if not args.skip_env_setup:
        ...
        install_torch_cu124()
        # NOTE: vLLM 0.19.0 is the first release with Gemma 4 support.
        install_vllm(data_dir, version="0.19.0")
        # NOTE: Install transformers >= 5.5.0 AFTER vLLM to override vLLM's transformers<5 pin.
        install_transformers_for_gemma4()
