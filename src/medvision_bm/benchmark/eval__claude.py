...
def parse_args():
    ...
    parser.add_argument(
        "--anthropic_model_code",
        required=True,
        type=str,
        help=(
            "Claude model code. For --api_provider anthropic, use an Anthropic model ID "
            "(e.g. claude-fable-5), see https://platform.claude.com/docs/en/about-claude/models/overview. "
            "For --api_provider openrouter, use an OpenRouter model ID (e.g. anthropic/claude-opus-4.8), "
    ...
    parser.add_argument(
        ...
        type=str,
        help="API provider to access the Claude model: 'anthropic' (direct) or 'openrouter'.",
    )
    parser.add_argument(
        "--model_name",
        required=True,
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "Enable adaptive thinking (default: enabled). Use --no-thinking to disable; "
            "the thinking parameter is then omitted entirely (Claude Fable 5 rejects an explicit 'disabled')."
        ),
    )
    parser.add_argument(
        "--max_tokens",
...
def main():
    ...
    if not os.environ.get(api_key_env_var, "").strip():
        raise EnvironmentError(
            f"{api_key_env_var} is not set. Export it before running the Claude evaluation with --api_provider {args.api_provider}."
        )

    # Configuration
    anthropic_model_code = args.anthropic_model_code
    ...
    if not args.skip_env_setup:
        ...
        install_vendored_lmms_eval(proj_dependency="claude")
    ...
    for task in tasks:
        ...
        # lmms_eval_specific_kwargs, and claude.anthropic_resized_hw() normalizes it
        # ("anthropic/" prefix stripped, "." -> "-") for cap lookup -- normalization lives
        # in exactly one place (claude._normalize_model_code).
        model_args = (
            f"model={anthropic_model_code},"
            f"provider={args.api_provider},"
            f"model_hf={anthropic_model_code},"
