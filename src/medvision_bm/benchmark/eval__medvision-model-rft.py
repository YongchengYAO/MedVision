...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        help="lmms-eval model module name to use for inference (e.g. vllm_qwen25vl, vllm_qwen3vl).",
    )
    parser.add_argument(
        "--model_hf_id",
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Inject the verl RFT training system prompt as the first message for every request. "
            "Use this when evaluating models fine-tuned with verl GRPO to match the training distribution."
        ),
    )
...
def main():
    ...
    for task in tasks:
        ...
        rc = run_evaluation_for_task_vllm_proxy(
            lmmseval_module=args.lmms_eval_module,
