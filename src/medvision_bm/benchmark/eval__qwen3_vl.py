...
def install_transformers_for_qwen3vl(transformers_version="4.57.0"):
    # NOTE: Reinstall transformers to the version required by Qwen3-VL.
    # NOTE: This overwrites any potentially incompatible version pulled in by other deps.
    subprocess.run(
        [
            sys.executable,
            "-m",
...
def resolve_qwen3vl_hf_overrides(model_hf_id: str) -> dict:
    """Build minimal HF config overrides required by vLLM for Qwen3-VL."""
...
def parse_args():
    ...
    # Sampling parameters.
    # NOTE: Qwen3-VL "Thinking" models MUST sample (greedy/temperature=0 makes them stop early
    # inside <think> before emitting <answer>). Defaults below mirror the model's
    # generation_config.json (temperature=0.8, top_k=20, top_p=0.95). Generation uses a fixed
    # seed internally for reproducibility.
    parser.add_argument(
        "--temperature",
        default=0.8,
        type=float,
        help="Sampling temperature. Do NOT set to 0 for Thinking models.",
    )
    parser.add_argument(
        "--top_p",
        default=0.95,
...
def main():
    ...
    if not args.skip_env_setup:
        ...
        # NOTE: vLLM 0.11.0 is the first release with Qwen3-VL support; adjust if needed for
        # compatibility with transformers 4.57.0 in your environment.
        install_vllm(data_dir, version="0.11.0")

        # NOTE: Reinstall transformers to overwrite potentially incompatible versions
        install_transformers_for_qwen3vl(transformers_version="4.57.0")
    ...
    hf_overrides = resolve_qwen3vl_hf_overrides(model_hf)
