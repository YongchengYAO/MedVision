"""
Checkpointed / resumable variant of build_parquet_ds.py.

Key differences vs build_parquet_ds.py:
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "(checkpointed / resumable variant, no test split)."
        )
    )
    # -- Model identifier
    parser.add_argument(
...
def build_parquet_dataset_checkpointed(**kwargs):
    ...
    # Parse limits (reuse the same helper as build_parquet_ds.py).
    (
        train_limit_AD,
        val_limit_AD,
        train_limit_detect,
    ...
    # Compute output directory (same naming convention as build_parquet_ds.py).
    ...
    # Mirror the shuffle + global-limit logic from build_parquet_ds.py.
    ...
    if ckpt["val_done"]:
        ...
        # Apply global val limit (mirror build_parquet_ds.py logic).
        val_limit = kwargs.get("val_sample_limit", 100)
        if val_limit > 0:
