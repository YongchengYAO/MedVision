"""
LoRA (QLoRA NF4) variant for Qwen3-VL, CoT multi-task SFT.

Mirrors train__SFT-CoT__qwen2_5_vl.py; the only model-specific piece would be the
collate function, but Qwen3-VL shares the Qwen ChatML chat format
(<|im_start|>/<|im_end|>) and exposes the same processor.image_token as
Qwen2.5-VL, so make_collate_fn_Qwen25VL (pad/image-token masking + completion-only
masking of non-assistant turns via mask_non_assistant_turns) is reused unchanged.
...
Multi-GPU usage:
  > accelerate launch -m medvision_bm.sft.train__SFT-CoT__qwen3vl [args]
"""
