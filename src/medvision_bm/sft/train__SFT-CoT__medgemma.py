"""
LoRA (QLoRA NF4) variant for MedGemma, CoT multi-task SFT.

Mirrors train__SFT-CoT__qwen2_5_vl.py; the only model-specific piece is the
collate function. MedGemma (Gemma 3 architecture) uses make_collate_fn_MedGemma
(pad + begin/end-of-image + image-token masking). Image processing on the dataset
side is dispatched by model_family_name ("medgemma") in get_resized_img_shape()
(fixed 896x896).

NOTE: the existing dev/train__SFT__medgemma.py is an older LoRA variant; this is
the CoT multi-task counterpart, parallel to train__SFT-CoT__qwen2_5_vl.py.

Key properties (same as the Qwen2.5-VL LoRA script, via prepare_trainer):
  - 4-bit NF4 base (BitsAndBytesConfig) + LoRA r=16 all-linear
  - DDP via per-rank device_map (no FSDP)
  - QLoRA recipe: lr 2e-4, linear scheduler
  - Optional post-training merge into the base model (merge_models)

Multi-GPU usage:
  > accelerate launch -m medvision_bm.sft.train__SFT-CoT__medgemma [args]
"""
