...
from medvision_bm.sft.qwen25vl_tooluse_utils import make_collate_fn_Qwen25VL_tooluse
from medvision_bm.sft.sft_utils import (
    _format_data_AngleDistanceTask_tooluse,
    _format_data_TumorLesionTask_tooluse,
    parse_sample_limits,
...
pg_kwargs = InitProcessGroupKwargs(timeout=datetime.timedelta(hours=3))

# pg_kwargs above is unused by HuggingFace Trainer (it creates its own Accelerator
# internally).  Patch the module-level default so init_process_group() picks it up
# before Trainer calls create_accelerator_and_postprocess().  Without this the
# 30-minute PyTorch default applies, which is too short for FSDP FULL_STATE_DICT
# saves to slow PVC storage (~33 GB FP32 model + optimizer states per checkpoint).
import torch.distributed.distributed_c10d as _dist_c10d
_dist_c10d._DEFAULT_PG_TIMEOUT = pg_kwargs.timeout
    ...
    if not kwargs.get("merge_only"):
        ...
        if kwargs.get("prepared_ds_dir") is not None:
            ...
            if new_shape_hw is not None:
                prepared_ds_dir += f"__resized-wh-{new_shape_hw[1]}x{new_shape_hw[0]}-tooluse"
            else:
                prepared_ds_dir += f"__original-tooluse"
            ...
            if not kwargs.get("skip_process_dataset"):
                ...
                if kwargs.get("tasks_list_json_path_AD") is not None:
                    ...
                    dataset_AD = prepare_dataset(
                        ...
                        mapping_func=_format_data_AngleDistanceTask_tooluse,
                ...
                if kwargs.get("tasks_list_json_path_TL") is not None:
                    ...
                    dataset_TL = prepare_dataset(
                        ...
                        mapping_func=_format_data_TumorLesionTask_tooluse,
                        model_family_name=model_family_name,
                        base_model_hf=base_model_hf,
                        num_workers_concat_datasets=kwargs.get(
                            "num_workers_concat_datasets"
        ...
        trainer = prepare_trainer_fullFT(
            ...
            data=dataset,
            make_collate_fn=make_collate_fn_Qwen25VL_tooluse,
            ...
            push_model=kwargs.get("push_LoRA"),
...
if __name__ == "__main__":
    ...
    # parse_validate_args_multiTask yields lora_checkpoint_dir; remap to checkpoint_dir
