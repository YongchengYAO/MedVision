...
Prerequisites (bundled copy for the `medvision` skill, runs from any directory):
  ...
  - `medvision_ds` installed: only the per-label aggregation needs it; without it every
    label lookup fails silently and the per-model summary JSON comes out empty while the
    per-sample *_proc_acc.jsonl is still complete.
  - The dataset tree present at the absolute `doc["landmark_file"]` paths recorded at
    evaluation time (GT landmarks are read from those files, not from HuggingFace).
...
try:  # bundled-skill guard: report the missing package instead of a bare traceback
    ...
    raise SystemExit(
        f"[analyze_process_accuracy_TL] cannot import medvision_bm ({_e}).\n"
