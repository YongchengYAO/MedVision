#!/usr/bin/env python3
"""Render the final Clinical Decision Agreement (CDA) report into this folder.

Every other CDA output lands in the gitignored ``Results/`` tree, so a fresh
clone carries the analysis code but none of its numbers. This script collects the
leaderboards into a single Markdown file that lives beside the code.
...
def build_report(args):
    ...
    L.append(
        ...
        "overwritten on every run of `run_CDA_analysis.sh`."
    )
    L.append(
        ...
        "`README.md` for the method and `DESIGN.md` for the implementation."
    )
    ...
    L.append(
        ...
        "one-sided test of that statistic > 0 inverted from the same resamples. "
        "Both are computed on **wκ** for an ordinal proxy — every proxy here — so "
        "read the interval against the `wκ` column, not `κ`.\n"
    ...
    L.append("```bash")
    L.append("REMOVED_SAMPLES_DIR=$PWD/Data/Datasets \\")
    if src:
        L.append(f"  CDA_PARSED_DIR={args.parsed_dirname} \\")
    L.append("  bash script/analyze/clinical-decision-analysis/run_CDA_analysis.sh")
    L.append("```")
    L.append(
        "\nRun from the repo root. It needs the gitignored `Results/` and `Data/` "
        "trees, so a bare clone cannot reproduce it."
    )
    return "\n".join(L) + "\n"
