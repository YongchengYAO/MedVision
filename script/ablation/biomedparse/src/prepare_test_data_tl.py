...
try:
    from medvision_bm.sft.sft_utils import _load_resize_nifti_2d, _load_single_dataset, normalize_img
...
def _doc_to_visual_adaptive_norm(doc, new_shape_hw=None):
...
def __get_image_info(doc, processor_module="preprocess_biometry"):
...
def process_one_sample(args):
    ...
    try:
        label = str(doc["label"])
        _, label_name = __get_image_info(doc)

        visuals = _doc_to_visual_adaptive_norm(doc)
        pil_img = visuals[0]
...
def prepare_data(tasks_json, output_dir, num_workers=None, limit_per_subtask=-1, filter_dataset=None):
    ...
    for task_key in tasks_dict:
        if filter_dataset is not None and task_key.split("_TumorLesionSize_")[0] not in _wanted:
            continue
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Prepare NPZ test data for BiomedParse TL, loading from HuggingFace "
