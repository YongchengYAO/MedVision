...
def _draw_detection_overlay_on_ax(ax, doc, sample):
    ...
    if gt_bbox and len(gt_bbox) == 4:
        ...
        ax.add_patch(mpatches.Rectangle(
            (c0, r0), c1 - c0, r1 - r0,
            linewidth=4, edgecolor=C_GT_BOX, facecolor='none', linestyle='--', zorder=2,
        ))
    ...
    if pred_bbox:
        ...
        ax.add_patch(mpatches.Rectangle(
            (c0, r0), c1 - c0, r1 - r0,
            linewidth=4, edgecolor=C_PRED_BOX, facecolor='none', linestyle='-', zorder=3,
        ))
...
def _build_metrics_tokens(sample):
    ...
    if gt_bbox and len(gt_bbox) == 4:
        gt_str = f"[{gt_bbox[0]:.3f}, {gt_bbox[1]:.3f}, {gt_bbox[2]:.3f}, {gt_bbox[3]:.3f}]"
    else:
        gt_str = "N/A"
    ...
    tokens += _tokenize_line(f"Prediction:    bbox = {pred_str_fmt}", highlight=pred_color)
