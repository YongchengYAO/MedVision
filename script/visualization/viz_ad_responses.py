...
_PROMPT_HEADERS = frozenset({
    'Task:', 'Additional information:', 'Format requirement:', 'Reasoning steps:',
})
...
def _extract_ad_color_maps(raw_text):
    ...
    if m:
        txt = m.group(1)
        pm = re.search(r'\(pixel_width.*?\)\s*=\s*\(([^)]+)\)', txt) \
          or re.search(r'\(pw,\s*ph\)\s*=\s*\(([^)]+)\)', txt)
        if pm:
            pixel_strs.update(_NUM_RE.findall(pm.group(1)))
        im = re.search(r'\(image_width.*?\)\s*=\s*\(([^)]+)\)', txt) \
          or re.search(r'\(W,\s*H\)\s*=\s*\(([^)]+)\)', txt)
        if im:
...
def _pick_num_color(n, ctx_step, ctx_part, in_final_answer,
                    coord_map, result_map, pixel_strs, image_strs):
    if in_final_answer:
...
def _colorize_segment(text, ctx_step, ctx_part, in_final_answer,
                       coord_map, result_map, pixel_strs, image_strs):
    out = []
    for piece in re.split(r'(\d+\.?\d*)', text):
        ...
        if re.fullmatch(r'\d+\.?\d*', piece):
            c = _pick_num_color(piece, ctx_step, ctx_part, in_final_answer,
                                coord_map, result_map, pixel_strs, image_strs)
...
def _add_ad_number_colors(tokens, coord_map, result_map, pixel_strs, image_strs):
    ...
    for seg, color, bold in tokens:
        ...
        if color == C_TEXT and (ctx_step is not None or in_final_answer):
            result.extend(_colorize_segment(
                seg, ctx_step, ctx_part, in_final_answer,
                coord_map, result_map, pixel_strs, image_strs,
            ))
        else:
...
def _tokenize_estimate_clause(text):
    ...
    for i, part in enumerate(parts):
        ...
        if _ENTITY_SPLIT_RE.fullmatch(part):
            ...
            if m:
                if m.group(1): tokens.append((m.group(1), C_TEXT,       False))
                if m.group(2): tokens.append((m.group(2), C_LABEL_NAME, False))
                if m.group(3): tokens.append((m.group(3), C_TEXT,       False))
            else:
                m2 = re.match(r'(\s*)(.*)', part, re.DOTALL)
                if m2.group(1): tokens.append((m2.group(1), C_TEXT,       False))
                if m2.group(2): tokens.append((m2.group(2), C_LABEL_NAME, False))
        else:
...
def _tokenize_task_line(line):
    ...
    return [
        ...
        *_tokenize_estimate_clause(m.group(3)),  # ", estimate ... (landmark/line N) <name> ..."
    ]
...
def _preprocess_response(s):
    for pat in [
        r'</?think>', r'</?step-\d+-reasoning>', r'</?step-\d+-answer>',
        r'</?answer>', r'</?tool_call>', r'</?tool_response>',
    ]:
        s = re.sub(r'[^\S\n]*(' + pat + r')[^\S\n]*', r'\n\1\n', s)
    s = re.sub(r'\n{2,}', '\n', s)
    return s.strip()
        ...
        for seg, color, bold in line_segs:
            ...
            for wi, word in enumerate(words):
                ...
                if x > x0 and x + need_w > max_x:
                    ...
                    need_w = len(piece) * char_w_ax
                if piece:
                    ax.text(x, y, piece, color=display_color, fontsize=fontsize,
                            fontweight=fw, fontfamily='monospace',
                            transform=ax.transAxes, va='top', ha='left', clip_on=True)
                    x += need_w
        y -= line_h_ax
...
def _draw_ad_overlay_on_ax(ax, doc, proc_acc):
    ...
    """
    try:
        _, image_2d = _load_nifti_2d(doc['image_file'], doc['slice_dim'], doc['slice_idx'])
    ...
    if proc_acc:
        ...
        if metric_type == 'distance':
            ...
            if gt1 and gt2:
                ...
                ax.plot([c1, c2], [r1, r2], color=C_GT_MAJOR, ls='--', linewidth=4, zorder=2)
                ax.scatter([c1, c2], [r1, r2],
            ...
            if pred1 and len(pred1) == 2 and pred2 and len(pred2) == 2:
                ...
                ax.plot([pc1, pc2], [pr1, pr2], color='#F37020', ls='-', linewidth=3, zorder=3)
                ax.scatter([pc1, pc2], [pr1, pr2],
                           color=[_DOT_COLORS[2], _DOT_COLORS[3]],
                           edgecolors='black', s=60, linewidth=1.5, zorder=4)

            legend_handles = [
                mlines.Line2D([], [], color=C_GT_MAJOR, ls='--', linewidth=3, label='GT landmarks'),
                mlines.Line2D([], [], color='#F37020',  ls='-',  linewidth=3, label='Pred landmarks'),
            ]
            ...
            for j, (cx, ry) in enumerate(all_pts[:4]):
                ax.scatter(cx, ry, color=_DOT_COLORS[j],
                           edgecolors='black', s=60, linewidth=1.5, zorder=4)

            legend_handles = [
                mlines.Line2D([], [], color=C_GT_MAJOR, ls='--', linewidth=3, label='GT line 1'),
                mlines.Line2D([], [], color=C_GT_MINOR, ls='--', linewidth=3, label='GT line 2'),
                mlines.Line2D([], [], color='#F37020',  ls='-',  linewidth=3, label='Pred line 1'),
                mlines.Line2D([], [], color='#FBBC05',  ls='-',  linewidth=3, label='Pred line 2'),
            ]

    if legend_handles:
        ax.legend(handles=legend_handles, loc='upper right', fontsize=14,
                  facecolor='white', edgecolor='#D1D5DB', framealpha=0.85)
    ax.axis('off')
...
def _build_metrics_tokens(sample, proc_acc, eq_acc=None):
    ...
    metric_type = (proc_acc or {}).get('metric_type') \
                  or sample['doc']['biometric_profile']['metric_type']
    ...
    if proc_acc:
        ...
        if eq_acc:
            ...
            tokens += _tokenize_line(line, highlight={ma_str: C_MAJ_LEN} if ma is not None else {})
...
def _make_filename(sample, dataset_name):
    ...
    return (f"{dataset_name}__Task{task_id:02d}"
...
def _plot_sample(sample, proc_acc, out_path, reshape_hw, eq_acc=None):
    ...
    col1_content_h = (HDR_H + in_h) + 2*BOX_GAP + (HDR_H + rs_h) + BOX_GAP + (HDR_H + mt_h)
    ...
    y      = OUTER_BOT
    mt_bot = tf(y); mt_hf = tf(HDR_H + mt_h); y += HDR_H + mt_h + BOX_GAP
    rs_bot = tf(y); rs_hf = tf(HDR_H + rs_h); y += HDR_H + rs_h + 2*BOX_GAP
    in_bot = tf(y); in_hf = tf(HDR_H + in_h)

    hkw = dict(transform=fig.transFigure, fontsize=22, fontweight='bold',
               color=C_TEXT, va='bottom', fontfamily='monospace', zorder=0)
    top_pad = tf(0.02)
    fig.text(_IX + 0.006, in_bot + in_hf - tf(HDR_H * 0.3), "[ Prompt ]",                    **hkw)
    fig.text(_IX + 0.006, rs_bot + rs_hf - tf(HDR_H * 0.3), "[ Response ]",                  **hkw)
    fig.text(_IX + 0.006, mt_bot + mt_hf - tf(HDR_H * 0.3), "[ GT / Prediction / Metrics ]", **hkw)
    ...
    ax_in.set_facecolor('none'); ax_in.axis('off')
    _draw_tokens(ax_in, in_tokens, 0.002, 0.998,
                 FONTSIZE_IN, COL1_AX_FRAC * FIG_W, (in_hf - tf(HDR_H) - top_pad) * fig_h)

    ax_rs = col1_axes(rs_bot, rs_hf)
    ax_rs.set_facecolor('none'); ax_rs.axis('off')
    _draw_tokens(ax_rs, rs_tokens, 0.002, 0.998,
                 FONTSIZE_RS, COL1_AX_FRAC * FIG_W, (rs_hf - tf(HDR_H) - top_pad) * fig_h)

    ax_mt = col1_axes(mt_bot, mt_hf)
    ax_mt.set_facecolor('none'); ax_mt.axis('off')
    _draw_tokens(ax_mt, mt_tokens, 0.002, 0.998,
                 FONTSIZE_MT, COL1_AX_FRAC * FIG_W, (mt_hf - tf(HDR_H) - top_pad) * fig_h)

    # Separator line at bottom of section 1 (Prompt)
    fig.add_artist(mlines.Line2D(
        [_IX, 1.0 - LM], [in_bot, in_bot],
        transform=fig.transFigure, color=C_SEP, linewidth=1.5, zorder=0,
    ))
    ...
    _sd = doc['slice_dim']
    if _sd == 0:   _ovl_H, _ovl_W = _nii_shape[1], _nii_shape[2]
    elif _sd == 1: _ovl_H, _ovl_W = _nii_shape[0], _nii_shape[2]
    else:          _ovl_H, _ovl_W = _nii_shape[0], _nii_shape[1]
    ...
    ax_img = fig.add_axes([panel_x_frac, max(0.0, sec1_center - img_h_frac / 2),
                           panel_w_frac, img_h_frac])
    ...
    ax_ovl = fig.add_axes([panel_x_frac, max(0.0, sec23_center - ovl_h_frac / 2),
                           panel_w_frac, ovl_h_frac])
...
def main():
    ...
    parser.add_argument("--reshape_hw",         nargs=2, type=int, default=None, metavar=("H", "W"))
    parser.add_argument("--limit",              type=int, default=None)
    parser.add_argument("--limit_per_jsonl",    type=int, default=None,
    ...
    parser.add_argument("--removed_samples_filename", default="multi_cluster_samples_v1.0.0_to_v1.1.0.json",
    ...
    if args.model_dir:
        ...
        all_jsonls = [f for f in all_jsonls if not f.endswith("_proc_acc.jsonl") and not f.endswith("_eq_acc.jsonl")]
        if not all_jsonls:
        ...
        for jsonl_path in all_jsonls:
            ...
            if args.removed_samples_dir:
                json_path = os.path.join(args.removed_samples_dir, dataset_name,
                                         args.removed_samples_filename)
                if os.path.exists(json_path):
                    removed_set = _build_removed_set(json_path)
                    print(f"[Info] Loaded removed-samples filter: {json_path} ({len(removed_set)} entries)")
                    before = len(samples)
                    samples = [
                        s for s in samples
                        if (_relative_image_file(s['doc'].get('image_file', ''), dataset_name),
                            s['doc'].get('slice_dim'),
                            s['doc'].get('slice_idx'),
                            int(s['doc'].get('taskID', 0))) not in removed_set
                    ]
                    print(f"[Info] Filtered out {before - len(samples)} removed samples, {len(samples)} remaining")
            ...
            print(f"[{os.path.basename(jsonl_path)}] {len(samples)} sample(s) → {per_file_out_dir}")
            for i, sample in enumerate(samples):
                doc_id = sample['doc_id']
                fname  = _make_filename(sample, dataset_name)
                out_path = os.path.join(per_file_out_dir, fname)
                out = _plot_sample(sample, proc_acc_by_id.get(doc_id), out_path, reshape_hw, eq_acc=eq_acc_by_id.get(doc_id))
                if out:
        ...
        if args.removed_samples_dir:
            ...
            if dataset_name:
                ...
                print("[Warning] Could not extract dataset name from JSONL filename; filtering skipped")
