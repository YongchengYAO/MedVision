...
from medvision_bm.utils.configs import landmark_dot_colors as _DOT_COLORS
from medvision_bm.utils.configs import response_coord_colors as _COORD_COLORS
...
def _draw_ad_overlay_on_ax(ax, doc, proc_acc):
    """
    Draw the original-resolution image with GT landmarks/lines (dashed) and predicted
    landmarks/lines (solid) + dots, rotated 90° CCW to match the webpage renderer:
    imshow(image_2d.T, origin='lower', aspect=psz[1]/psz[0]). Model rel-coord (x_rel, y_rel,
    lower-left origin) maps to rotated plot space as (plot_x=H*(1-y_rel), plot_y=x_rel*W).
    """
    try:
    ...
    if proc_acc:
        ...
        if metric_type == "distance":
            ...
            if gt1 and gt2:
                px1, py1 = to_plot(gt1[0], gt1[1])
                px2, py2 = to_plot(gt2[0], gt2[1])
                ax.plot([px1, px2], [py1, py2], color=C_GT_MAJOR, ls="--", linewidth=4, zorder=2)
                ax.scatter([px1, px2], [py1, py2], color=[_DOT_COLORS[0], _DOT_COLORS[1]], edgecolors="black", s=80, linewidth=1.5, zorder=4)

            pred1 = proc_acc.get("step1_pred")
            pred2 = proc_acc.get("step2_pred")
            if pred1 and len(pred1) == 2 and pred2 and len(pred2) == 2:
                ppx1, ppy1 = to_plot(pred1[0], pred1[1])
                ppx2, ppy2 = to_plot(pred2[0], pred2[1])
                ax.plot([ppx1, ppx2], [ppy1, ppy2], color="#F37020", ls="-", linewidth=3, zorder=3)
                ax.scatter([ppx1, ppx2], [ppy1, ppy2], color=[_DOT_COLORS[2], _DOT_COLORS[3]], edgecolors="black", s=60, linewidth=1.5, zorder=4)
            ...
            for j, (px, py) in enumerate(all_pts[:4]):
                ax.scatter(px, py, color=_DOT_COLORS[j], edgecolors="black", s=60, linewidth=1.5, zorder=4)
