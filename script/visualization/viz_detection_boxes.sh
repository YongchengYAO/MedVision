#!/bin/bash
# Visualize MedVision Detection task predictions (GT + model-predicted bounding boxes).
#
# Usage:
#   TASK_DIR=<path>  bash viz_detection_boxes.sh
#   MODEL_DIR=<path> bash viz_detection_boxes.sh
#
...
#
# Output formats (resolved by viz_detection_boxes.py):
...
python "$SCRIPT_DIR/viz_detection_boxes.py" \
    "${INPUT_ARGS[@]}" \
    --fig_dir "$FIG_DIR" \
    --save_as_pdf \
    $LIMIT_ARG
