...
def _draw_tl_overlay_on_ax(ax, doc, proc_acc):
    ...
    if mask_path and os.path.exists(mask_path):
        try:
            ...
            if mask_bin.any():
                ax.contour(mask_bin, levels=[0.5], colors='#2ECC71', linewidths=4, zorder=1)
        except Exception:
            pass
    ...
    if proc_acc:
        ...
        for j, (cx, ry) in enumerate(all_pts[:4]):
            ax.scatter(cx, ry, color=_DOT_COLORS[j], edgecolors='black',
                       marker='o', s=60, linewidth=1.5, zorder=4)

    legend_handles = [
        mlines.Line2D([], [], color=C_GT_MAJOR, ls='--', linewidth=3, label='GT major axis'),
        mlines.Line2D([], [], color=C_GT_MINOR, ls='--', linewidth=3, label='GT minor axis'),
        mlines.Line2D([], [], color='#F37020', ls='-',  linewidth=3, label='Pred major axis'),
        mlines.Line2D([], [], color='#FBBC05', ls='-',  linewidth=3, label='Pred minor axis'),
    ]
    ...
    if args.model_dir:
        ...
        if args.removed_samples_dir:
            ...
            if dataset_name:
                ...
                print(f"[Warning] Could not extract dataset name from JSONL filename; filtering skipped")
