...
#   base_url https://api.moonshot.ai/v1. China users: export MOONSHOT_BASE_URL=https://api.moonshot.cn/v1
# - openrouter: set api_provider="openrouter" and use an OpenRouter model ID,
#   e.g. kimi_model_code="moonshotai/kimi-k2.6" (https://openrouter.ai/models);
#   requires OPENROUTER_API_KEY instead of MOONSHOT_API_KEY.
#api_provider="moonshot"          # direct Moonshot API; requires MOONSHOT_API_KEY
#kimi_model_code="kimi-k2.6"
api_provider="openrouter"          # OPENROUTER_API_KEY is set in this env; Moonshot key is not
kimi_model_code="moonshotai/kimi-k2.6"   # exact OpenRouter slug (multimodal)

# API key check + sanitization (pod-injected env vars can carry a trailing newline,
# which breaks HTTP auth headers)
if [ "${api_provider}" = "moonshot" ]; then
...
python -m medvision_bm.benchmark.install_vendored_lmms_eval --lmms_eval_opt_deps kimi
pip install -r "${benchmark_dir}/requirements/requirements_eval_kimi.txt" --no-deps
